...
class Network:
  ...
  def __init__(self,cfgfilename, conn, agent):
    ...
    for i in range(len(intersections)):
      ...
      conn.trafficlight.setRedYellowGreenState(intersections[i], "rGrrrrrGrrrr")
  ...
  def DQN_getstate(self, conn, action):
    ...
    num_each_lane_arr = num_each_lane_arr.reshape(1, 8, 1)
...
def trafficlight_phase(list_links, light_list, agent):  #putting the link, phase, and light into a matrix
  ...
  for i in range(len(list_links)):
      if list_links[i][0][3] == list_links[i-1][0][3] and list_links[i][0][6] == list_links[i-1][0][6] and list_links[i][1][3] == list_links[i-1][1][3] and list_links[i][1][6] == list_links[i-1][1][6]:
          a -= 1
